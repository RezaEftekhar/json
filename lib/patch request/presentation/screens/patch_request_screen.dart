    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          BlocConsumer<PatchRequestBloc, PatchRequestState>(
            listener: (context, state) {
              if (state is PatchAllRequestsCompleted) {
                Navigator.push(
                    context,
                    MaterialPageRoute(
            ...
            builder: (context, state) {
              return state is PatchRequestLoading
                  ? const Center(child: CircularProgressIndicator())
                  : Padding(
                      padding: const EdgeInsets.all(16),
                      child: ElevatedButton(
                          onPressed: () {
                            BlocProvider.of<PatchRequestBloc>(context)
                                .add(PatchAllRequests(
